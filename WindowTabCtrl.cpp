...
void CWindowTabCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	pOldFont=	dc.SelectObject(&m_SelFont);
		rcTab.top -= ::GetSystemMetrics(SM_CYEDGE);
	} 
	else 
	{
		
	pOldFont=	dc.SelectObject(&m_UnselFont);
	}
	
	dc.DrawText(szLabel, rcTab, DT_SINGLELINE | DT_VCENTER | DT_CENTER|DT_END_ELLIPSIS);
...
/*	CPopupMenu cMenu;
	...
	cMenu.AppendMenu(MF_STRING, ID_WINDOW_CLOSE, _T("&Закрыть"));
    ...
    cMenu.InsertMenu(0, MF_BYPOSITION | MF_STRING | MF_DISABLED, 0, _T("Активировать окно"));

    ::SetMenuDefaultItem(cMenu.m_hMenu, 0, TRUE);
*/
//	CInfoFrame* pMainWnd=(CInfoFrame*)AfxGetMainWnd();
		CMenu* pSysMenu = pChild->GetSystemMenu(FALSE);
		//pSysMenu->ModifyMenu (4,MF_BYPOSITION,11111,"Maximize");
	if (pSysMenu != NULL)
	{
		
	pSysMenu->TrackPopupMenu(TPM_LEFTALIGN | TPM_RIGHTBUTTON, ptScreen.x, ptScreen.y, /*pMainWnd*/pChild);
