...
public:
	long GetContentOfCash(double* dblCash);
	long CloseDay(int nPassw);
	long PrintCheck(CTableValue* tblGoods,CY& cySummCheck,CY& cySummCash,BOOL blIsReturn,long nDepartment,long* numCheck);
	long SalePosition(const long nDepartment,const CY& cySumm,const double dblQty);
	long CloseCheck(const CY& cySummCheck,const CY& cyCash);
	long ReturnSalePosition(const long nDepartment,const CY& cySumm,const double dblQty);
	long DisconnectFR();
	long ConnectFR(int nPassw);
	long BreakStringAndPrint(LPCTSTR strName,int lenght);
	long PrintStringInMiddle(LPCTSTR strName,int lenght);
	long PrintString(LPCTSTR strPrint);
	long ErrorPrintProcessing();
	CString m_strResultCodeDescription;
	CString GetResultCodeDescription();
	long ErrorProcessing();
	void ShowProperties();
		IDrvFR47 drv;
