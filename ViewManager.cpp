...
//
void AddMenuTitle(CMenu* popup, LPCTSTR title)
{
...
{
	CMDIFrameWnd* pFrame = static_cast<CMDIFrameWnd*>(AfxGetThread()->m_pMainWnd);
	if (pFrame == NULL)
...
void CViewManager::OnViewManagerToolTip(NMHDR* pNMHDR, LRESULT* pResult)
{
    *pResult = 0;
	...
	if (nTab >= 0)
	{
		...
		ASSERT(pDoc != NULL);
		CString strTabText = pDoc->GetTitle ();*/
	CString strTabText;
		// scan the TabCtrl Tabs
        
			TCHAR szLabel[_MAX_PATH];
			
            CMDIChildWnd* pViewAt = GetTabInfo(nTab, szLabel);
            ASSERT_KINDOF(CMDIChildWnd, pViewAt);
			
			// Note that we don't use GetDocument()->GetTitle() here as it doesn't include
			// the :n suffix when a Doc is open in multiple views. eg. test.cpp:1
            pViewAt->GetWindowText(strTabText);
